...
const renderPrivacyPolicy = () => {
  ...
  const sections = [
    ...
    privacySection('privacy-current-site', 4, 'Обработка на текущем сайте', `<p>МИС не подключена, формы записи и поля ввода на сайте отсутствуют. Кнопка записи открывает телефонный диалог с официальными номерами клиники. Сайт не получает содержание телефонного разговора и не формирует заявку.</p><div class="privacy-policy__contact-list">${phoneLinks}<a href="${escapeHtml(CONTACTS.emailHref)}">${escapeHtml(CONTACTS.email)}</a></div><p>После звонка или отправки письма пользователь взаимодействует с оператором по выбранному каналу. Данные, добровольно сообщённые при таком обращении, могут обрабатываться для ответа, записи на приём, заключения и исполнения договора, исполнения обязанностей медицинской организации и защиты законных прав.</p><p>Специальные категории персональных данных и биометрические персональные данные средствами текущего сайта не собираются.</p>`),
    privacySection('privacy-technical-data', 5, 'Технические данные при обращении к сайту', '<p>Инфраструктура, обеспечивающая доступ к размещённой версии сайта, может автоматически получать IP-адрес, дату и время запроса, адрес запрошенной страницы, сведения о браузере и устройстве, код ответа и источник перехода. Эти сведения могут входить в технические журналы или серверные логи.</p><p>Такие данные используются только для доставки страниц, обеспечения работоспособности и безопасности, диагностики ошибок и предотвращения неправомерного доступа. На сайте нет внешней аналитики, рекламных пикселей, удалённых шрифтов, карт и сторонних виджетов.</p>'),
    ...
    privacySection('privacy-third-parties', 8, 'Передача и поручение обработки', '<p>Оператор не распространяет персональные данные неопределённому кругу лиц. Передача допускается с согласия субъекта, для исполнения договора, по требованию закона или уполномоченного органа, а также для защиты прав в предусмотренных законом пределах.</p><p>Оператор вправе поручить обработку подрядчику по договору, который устанавливает перечень данных, цели, действия, конфиденциальность и меры защиты. Передача данных граждан Российской Федерации с использованием сайта организуется с соблюдением требований к локализации баз данных. Трансграничная передача средствами текущего сайта не осуществляется.</p>'),
    ...
    privacySection('privacy-storage', 11, 'Локальные настройки браузера', '<p>Текущая версия сайта сохраняет в localStorage только записи <code>cookie-consent</code> и <code>accessibility-preferences</code>. Запись <code>accessibility-preferences</code> версии 2 содержит только настройки отображения и логическое значение голосовых подтверждений. Полный состав и способы удаления описаны на странице <a href="cookies.html">«Технические настройки и cookies»</a>.</p>'),
    privacySection('privacy-updates', 12, 'Изменение политики', '<p>Актуальная редакция постоянно доступна на этой странице. Оператор обновляет Политику при изменении законодательства, целей, состава данных или способов обработки.</p><p>До подключения МИС, формы онлайн-записи или другого нового канала сбора данных оператор обновит настоящую Политику, текст согласия и сведения о получателях и сроках обработки. Новая функция не публикуется до такого обновления.</p><p><strong>Редакция от 12 августа 2026 года.</strong></p>'),
  ].join('');

  return `<section class="section privacy-policy"><div class="container privacy-policy__layout"><div class="privacy-policy__overview"><article class="privacy-policy__summary"><span class="privacy-policy__summary-icon">${renderIcon('shield')}</span><div><p class="eyebrow">Оператор персональных данных</p><h2>${escapeHtml(CLINIC.shortLegalName)}</h2><p>Документ описывает фактическую работу сайта без подключённой МИС и веб-форм записи.</p></div></article><nav class="privacy-policy__contents" aria-labelledby="privacy-contents-title"><h2 id="privacy-contents-title">Содержание</h2><ol>${contents}</ol></nav></div><article class="privacy-policy__body">${sections}</article></div></section>`;
};
...
export const LEGAL_PAGES = Object.freeze([
  ...
  makePage({
    ...
    body: '<section class="section"><div class="container"><h2>Что хранится</h2><p>Сайт использует localStorage только с ключами <code>cookie-consent</code> и <code>accessibility-preferences</code>. <code>accessibility-preferences</code> — строго проверяемая JSON-запись версии 2. Она содержит только настройки отображения — режим, масштаб, цветовую схему, шрифт, интервалы и показ изображений — и логическое значение голосовых подтверждений.</p><p>Не сохраняются медицинские или контактные данные, текст произнесённых фраз и история выполненных действий. localStorage — это не HTTP-cookie и не файл cookie.</p><h2>Голосовые подтверждения</h2><p>Голос используется только для коротких подтверждений изменения настроек. Для них браузер использует только доступный локальный русский голос браузера. Страница целиком не читается. Сторонние виджеты или удалённые TTS-сервисы не загружаются, сетевого резервного сервиса нет.</p><h2>Чего на сайте нет</h2><p>Не загружаются аналитика, рекламные идентификаторы или пиксели, удалённые шрифты, карты и сторонние виджеты.</p><h2>Управление настройками</h2><p>Кнопка «Сбросить настройки» возвращает масштаб 100%, стандартную цветовую схему, фирменный шрифт и обычные интервалы, показывает изображения, выключает голосовые подтверждения и специальный режим. Если голос был включён, один раз произносится «Настройки сброшены», после чего голос остаётся выключенным. «Обычная версия сайта» отключает специальное отображение, но сохраняет выбранные предпочтения. Очистка данных сайта в браузере удаляет и <code>cookie-consent</code>, и <code>accessibility-preferences</code>. Значения также меняются при новом выборе или сбросе. Сам localStorage не передаёт эти данные клинике.</p><p>Баннер cookies запоминает только выбор в записи <code>cookie-consent</code>.</p><button class="button button-secondary" type="button" data-cookie-settings>Открыть настройки cookies</button></div></section>',
  }),
]);
