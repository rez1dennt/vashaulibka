...
const contactsSection = [
  ...
  `<p class="home-contact__address">${renderIcon('pin')}<span>${escapeHtml(CLINIC.activityAddress)}</span></p>`,
  `<div class="home-contact__phones">${CONTACTS.phones.map((phone) => `<a href="${phone.href}">${renderIcon('phone')}${escapeHtml(phone.label)}</a>`).join('')}</div>`,
  `<a class="home-contact__email" href="${CONTACTS.emailHref}">${renderIcon('mail')}${escapeHtml(CONTACTS.email)}</a>`,
  `<dl class="definition-list definition-list--compact">${hours}</dl><p><strong>${escapeHtml(HOURS.breakNote)}</strong></p>`,
