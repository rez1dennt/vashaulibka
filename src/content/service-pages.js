...
const serviceTabs = `<div class="services-tabs-view"><div class="service-tabs"><div role="tablist" aria-label="Лицензированные направления">${SERVICES.map((service, index) => `<button type="button" role="tab" id="services-tab-${service.slug}" aria-selected="${index === 0}" tabindex="${index === 0 ? 0 : -1}" aria-controls="services-panel-${service.slug}">${service.title}</button>`).join('')}</div><div>${SERVICES.map((service, index) => `<section class="card" id="services-panel-${service.slug}" role="tabpanel" aria-labelledby="services-tab-${service.slug}"${index === 0 ? '' : ' hidden'}><h2>${service.title}</h2>${serviceDetails(service)}</section>`).join('')}</div></div></div>`;

const serviceDisclosures = `<div class="services-disclosures-view">${SERVICES.map((service, index) => `<article class="card service-disclosure"><h2><button type="button" data-disclosure-button id="services-disclosure-${service.slug}" aria-expanded="${index === 0}" aria-controls="services-disclosure-panel-${service.slug}">${service.title}</button></h2><div class="disclosure-panel" id="services-disclosure-panel-${service.slug}" role="region" aria-labelledby="services-disclosure-${service.slug}"${index === 0 ? '' : ' hidden'}>${serviceDetails(service)}</div></article>`).join('')}</div>`;
...
const priceDisclosures = SERVICES.map((service, index) => `<article class="card price-disclosure"><h2><button type="button" data-disclosure-button id="price-disclosure-${service.slug}" aria-expanded="${index === 0}" aria-controls="price-panel-${service.slug}">${service.title}</button></h2><div class="disclosure-panel" id="price-panel-${service.slug}" role="region" aria-labelledby="price-disclosure-${service.slug}"${index === 0 ? '' : ' hidden'}>${serviceDetails(service)}</div></article>`).join('');
